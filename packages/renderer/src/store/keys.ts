...
export const useKeysStore = defineStore('keys', {
  ...
  getters: {
    withTTL(state) {
      return filter(state.list, key => key.ttl > -1)
    },
    current(state) {
      return find(state.list, {name: state.selected})
    },
    hasKeys(state) {
  ...
  actions: {
    setKeys(keys: Record<string, Key>) {
      this.list = fromPairs(sortBy(toPairs(create(keys)), 0))
    },
    addKey(key: Key) {
      this.list = fromPairs(sortBy(toPairs(create({...this.list, [key.name]: key})), 0))
    },
    removeKey(key: Key) {
