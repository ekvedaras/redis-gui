    ...
    <div class="overflow-y-auto h-full pb-10 rounded overflow-x-hidden">
      <div v-for="(item, i) in value" :key="i">
        <div class="relative" v-if="!isEditing[i]">
          <button type="button" @click="editItem(item, i)" class="absolute top-0 right-0 mt-2 mr-8 z-10">
            <EditIcon class="w-5 cursor-pointer text-gray-500 hover:text-redis"/>
          </button>
          <button type="button" @click="deleteItem(item, i)" class="absolute top-0 right-0 mt-2 mr-2 z-10">
